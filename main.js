...
function initTelemetry() {
  ...
  /* Live telemetry.
     First call POSTs to register this visit — exactly once per
     browser session (sessionStorage guard), so reloads and the
     30 s polls never inflate counts. Subsequent GETs are read-only. */
  async function fetchTelemetry(registerHit = false) {
    try {
      const res = await fetch(TELEMETRY_ENDPOINT, {
        method: registerHit ? "POST" : "GET",
      });
